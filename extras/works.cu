...
static Vector camera_up = Vector(0, 1, 0); // NEW: The camera's local up vector
...
Vector rotate_around_axis(const Vector& v, const Vector& axis, float theta_rad) {
    Vector k = axis.normalized();
...
static inline void fit_to_bounds(const Point& min_pt, const Point& max_pt) {
    ...
    const double max_extent = std::max({dx, dy, dz, 1e-6});

    float camera_radius = static_cast<float>(max_extent * 1.5);
    min_radius    = static_cast<float>(max_extent * 0.05);
    max_radius    = static_cast<float>(max_extent * 10.0);

    // Set an initial camera position and up vector
    camera_lookfrom = camera_lookat + Vector(0.5, 0.5, 1.0).normalized() * camera_radius;
    camera_up = Vector(0, 1, 0); // Initialize up vector
...
void mouse_callback(GLFWwindow* /*window*/, double xpos, double ypos) {
    if (left_mouse_pressed) {
        ...
        Vector offset = camera_lookfrom - camera_lookat;

        // --- Horizontal Rotation (Yaw) ---
        // Rotate position and up vector around the WORLD up axis
        Vector world_up_axis(0, 1, 0);
        offset = rotate_around_axis(offset, world_up_axis, -dx * sensitivity);
        camera_up = rotate_around_axis(camera_up, world_up_axis, -dx * sensitivity);

        // --- Vertical Rotation (Pitch) ---
        // Rotate position and up vector around the LOCAL right axis
        Vector forward_vec = (camera_lookat - camera_lookfrom).normalized();
        Vector right_vec = cross(forward_vec, camera_up).normalized();
        
        offset = rotate_around_axis(offset, right_vec, -dy * sensitivity);
        camera_up = rotate_around_axis(camera_up, right_vec, -dy * sensitivity);

        // Update camera position
        camera_lookfrom = camera_lookat + offset;
        camera_changed = true;
    }
    last_mouse_x = xpos;
    last_mouse_y = ypos;
...
void scroll_callback(GLFWwindow* /*window*/, double /*xoffset*/, double yoffset) {
    Vector offset = camera_lookfrom - camera_lookat;
    float current_radius = offset.length();
    ...
    camera_lookfrom = camera_lookat + offset.normalized() * new_radius;
    camera_changed = true;
}
...
int main(int argc, char** argv) {
    ...
    if (argc >= 2 && argv[1] != nullptr) {
        ...
        std::cerr << "No model file provided, using demo scene.\n";
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (camera_changed) {
            // NOTE: You will need to update your Renderer and Camera classes
            // to accept the new `camera_up` vector.
        ...
        float quad_width, quad_height, x_offset, y_offset;
